        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <a href="https://github.com/digeesh038" target="_blank" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors">GitHub</a>
                </div>
            </div>
        </div>
    </nav>
...
<div x-show="activeTab === 'playground'" x-transition 
     x-data="{
        ...
        sendRequest() {
            try {
                const json = JSON.parse(this.playgroundJson);
                fetch('/api', {
                    method: 'POST',
                ...
                .then(r => r.json())
                .then(d => this.playgroundResult = JSON.stringify(d, null, 2))
                .catch(err => this.playgroundResult = '⚠️ Error: ' + err.message);
            } catch(e) {
                this.playgroundResult = '❌ Invalid JSON: ' + e.message;
            }
        }
     }">
    ...
    <div class="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
        ...
        <div class="grid lg:grid-cols-2 gap-8">
            ...
            <div>
                ...
                <textarea 
                    class="w-full h-64 p-4 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white font-mono text-sm" 
                    placeholder='Enter your JSON payload here...' 
                    x-model="playgroundJson">{
  "slug": "test-page",
  "components": [
    {
      "type": "TextSection",
      "props": {
        ...
        "animation": "fadeInUp"
            ...
            <div>
                ...
                <pre class="w-full h-64 p-4 bg-gray-900 text-green-400 rounded-lg overflow-auto text-xs"
                      x-text="playgroundResult || 'Response will appear here...'">
                </pre>
            </div>
        </div>
    </div>
